...
f_ans =  load_txt("./baseball/answer.txt")
f.write("var bb_ans = \"" + f_ans + "\";\n")

url = "http://lastdimension.woobi.co.kr/kakao/"
path = "/home/web/kakao/txts/"
day_t = ["월", "화", "수", "목", "금", "토", "일"]
day_kor = day_t[ day ]  # re date if night

text = load_txt(path + "humor.txt")
f.write( make_msg("humor_msg", write_n(text.replace("\r","")),"[\"keyboard\"] : kb_main") )
text = load_txt(path + "weekend.txt")
f.write( make_msg("weekend_msg", write_n(text.replace("\r","")),"[\"keyboard\"] : kb_main") )
## meal keyboard with date
date_text = str(date) + " (" + day_kor +") 식단. \\n"
f.write( make_msg("lunch_msg", date_text + "식당을 골라주세요", "[\"keyboard\"] : kb_lunch") )
f.write( make_msg("dinner_msg", date_text + "식당을 골라주세요", "[\"keyboard\"] : kb_dinner") )
